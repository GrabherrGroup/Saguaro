...
int main(int argc,char** argv)
{
  ...
  commandArg<int> covCmmd("-m","minimum coverage", 2);
  commandArg<int> minCmmd("-d","minimum disagreeing", 2);
  commandArg<string> chrName("-n","chromosome name","mult");
  ...
  P.registerArg(covCmmd);
  P.registerArg(minCmmd);
  P.registerArg(chrName);
  ...
  int cov = P.GetIntValueFor(covCmmd);
  string chr = P.GetStringValueFor(chrName);
  ...
  for (i=0; i<len; i++) {
    ...
    for (j=0; j<n; j++) {
	  ...
	  || line[j] == 'M' || line[j] == 'R' || line[j] == 'W' || line[j] == 'S' || line[j] == 'Y' || line[j] == 'K' )
	covered++;
    ...
    f.SetCompressed(k, line.c_str(), n, chr, i);
    cout << k << " " << " " << i;
    for (int x=0; x<n; x++) {
      cout << " " << line[x];
    }
