...
void HMMWord::Update(const HMMStateFrameScoreSource & a) {
  ...
  for (i=n-1; i>0; i--) {    
    ...
    //cout << "Frame " << i << " score=" << a.GetScore(m_stateName[i]) << endl;
  }
...
void HMMTraceback::GetBest(svec<HMMTracebackNode> & out,
  ...
  while (index1 != -1) {      
    ...
    //=========================================================================
    // BIG WARNING: This is a putative bug fix for an out-of-counds condition!!
    // However, the results are NOT bit-compatible with the previous version!!
    out[n].Set(exit, entry, score, name, word);     
